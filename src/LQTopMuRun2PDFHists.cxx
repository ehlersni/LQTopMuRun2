...
LQTopMuRun2PDFHists::LQTopMuRun2PDFHists(Context & ctx, const string & dirname, bool use_ntupleweights_, bool use_pdf_weights_): Hists(ctx, dirname), use_ntupleweights(use_ntupleweights_), use_pdf_weights(use_pdf_weights_){
  ...
  is_LO = m_oname.Contains("LQtoT") || m_oname.Contains("DYJets") || m_oname.Contains("QCD"); // m_oname.Contains("Diboson") ||
  ...
  take_ntupleweights =  use_ntupleweights && (!is_LO || m_oname.Contains("DYJets")) && (m_oname != "SingleTop_T_tWch" && m_oname != "SingleTop_Tbar_tWch" && m_oname != "DibosonNLO_ZZ_2L2Nu");
...
void LQTopMuRun2PDFHists::fill(const Event & event){
  ...
  if(is_mc){
    ...
    // bool is_mlq_reconstructed = (event.electrons->size() >= 1 && event.muons->size() >= 2 && charge_opposite);
    // bool is_mlq_reconstructed = (event.electrons->size() == 0 && event.muons->size() == 3 && fabs(sum_mu_charge) == 1);

    if(event.genInfo->systweights().size() == 0 && take_ntupleweights) throw runtime_error("In LQTopMuRun2PDFHists.cxx: Systweights in event.genInfo() is empty but ntupleweights shall be taken. Is this correct? In this case add exception to take_ntupleweights.");
    if(event.genInfo->systweights().size() != 0 && (is_LO && !m_oname.Contains("DYJets") && !m_oname.Contains("Diboson"))) throw runtime_error("In LQTopMuRun2PDFHists.cxx: Systweights in event.genInfo() is NOT empty but this IS a LO sample. Is this correct? In this case Thomas says the genInfo weight should be used. Add this sample to take_ntupleweights");
    ...
    if(take_ntupleweights){
      ...
      for(int i=0; i<100; i++){
        if(use_pdf_weights){
          ...
          hist(name)->Fill(ht,fillweight); // wird immer gefuellt

          if(ht <= 2900) hist(name2)->Fill(ht,fillweight); // wird immer gefuellt
          else hist(name2)->Fill(2900,fillweight);

          if(!is_mlq_reconstructed){
            if(ht <= 2900) hist(name4)->Fill(ht,fillweight); // wird gefuellt wen mlq nicht rekonstruiert wird
...
LQTopMuRun2PDFHists::~LQTopMuRun2PDFHists(){}
