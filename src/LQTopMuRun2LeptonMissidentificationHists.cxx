#include "../include/LQTopMuRun2LeptonMissidentificationHists.h"
...
LQTopMuRun2LeptonMissidentificationHists::LQTopMuRun2LeptonMissidentificationHists(Context & ctx, const string & dirname, bool is_ele_): Hists(ctx, dirname), is_ele(is_ele_){
...
void LQTopMuRun2LeptonMissidentificationHists::fill(const Event & event){
  ...
  if(event.electrons->size() >= 2){
    ...
    */
	...
	else throw runtime_error("In LQTopMuRun2LeptonMissidentificationHists.cxx: Uncovered combination of real and best electrons.");
      }
    }
	...
	for(unsigned int j=0; j<event.jets->size(); j++){
	  ...
	  if(dr <= 0.4){
	    if(event.electrons->size() > 3) throw runtime_error("In LQTopMuRun2LeptonMissidentificationHists.cxx: More than 3 electrons in the event when identifying THE ONE faking jet. In case of >1 fake electrons, there should also be >1 faking jet. Modifiy the procedure or cut on Nele <= 3.");
	    if(dr < dr_min){
	      idx_matched_jet = j;
	      dr_min = dr;
	...
	for(unsigned int i=0; i<event.muons->size(); i++){
	  ...
	  for(unsigned int j=0; j<event.jets->size(); j++){
	    ...
	    if(dr <= 0.4){
	      if(event.muons->size() > 1) throw runtime_error("In LQTopMuRun2LeptonMissidentificationHists.cxx: More than 1 muon in the event when identifying THE ONE faking jet. Procedure only works in cases of == 1 muon.");
	      if(dr < dr_min_mu){
	...
	else if(is_mc) throw runtime_error("In LQTopMuRun2LeptonMissidentificationHists.cxx: This MC-mu seems to be neither real nor fake. What is it?");
	...
	hist("pt_matchingjets_realele")->Fill(event.jets->at(idx_matched_jet_real_ele).pt(),weight);
      ...
      else if(is_mc) throw runtime_error("In LQTopMuRun2LeptonMissidentificationHists.cxx: This MC-ele seems to be neither real nor fake. What is it?");
    ...
    if(!is_ele){
      ...
      else if(event.muons->size() != 0) throw runtime_error("In LQTopMuRun2LeptonMissidentificationHists.cxx: There is more than 1 muon in the event. Fix this.");
...
LQTopMuRun2LeptonMissidentificationHists::~LQTopMuRun2LeptonMissidentificationHists(){}
