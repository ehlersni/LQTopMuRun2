...
namespace uhh2examples {
  ...
  class LQTopMuRun2FullSelectionControlRegionBTagAlpha: public AnalysisModule {
  public:

    explicit LQTopMuRun2FullSelectionControlRegionBTagAlpha(Context & ctx);
  ...
  private:
    ...
    unique_ptr<Hists> h_finalselection_alpha, h_jets_finalselection_alpha, h_ele_finalselection_alpha, h_mu_finalselection_alpha, h_event_finalselection_alpha, h_lumi_finalselection_alpha;
    ...
    TString alpha_path; //= "/nfs/dust/cms/user/ehlersni/LQTopMuRun2/fullselection/extrapolationsrcr/ExtrapolationSRCRFunction_TTbarDY.root";
    unique_ptr<TFile> alpha_file;
    unique_ptr<TGraphAsymmErrors> alpha;
  ...
  LQTopMuRun2FullSelectionControlRegionBTagAlpha::LQTopMuRun2FullSelectionControlRegionBTagAlpha(Context & ctx){
    cout << "Hello World from LQTopMuRun2FullSelectionControlRegionBTagAlpha!" << endl;
    for(auto & kv : ctx.get_all()) cout << " " << kv.first << " = " << kv.second << endl;


    // alpha_path = "/nfs/dust/cms/user/ehlersni/LQTopMuRun2/fullselection/extrapolationsrcr/ExtrapolationSRCRFunction_TTbarDY.root";
    string tmp = ctx.get("alphapath");
    alpha_path = tmp;
    ...
    alpha_file.reset(new TFile(alpha_path,"READ"));
    alpha.reset((TGraphAsymmErrors*)alpha_file->Get("Graph"));
    ...
    // applying alpha weights

    h_finalselection_alpha.reset(new LQTopMuRun2Hists(ctx, "finalselection_alpha"));
    h_jets_finalselection_alpha.reset(new JetHists(ctx, "Jets_finalselection_alpha"));
    h_ele_finalselection_alpha.reset(new ElectronHists(ctx, "Ele_finalselection_alpha"));
    h_mu_finalselection_alpha.reset(new MuonHists(ctx, "Mu_finalselection_alpha"));
    h_event_finalselection_alpha.reset(new EventHists(ctx, "Event_finalselection_alpha"));
    h_lumi_finalselection_alpha.reset(new LuminosityHists(ctx, "Lumi_finalselection_alpha"));
  ...
  */
  ...
  bool LQTopMuRun2FullSelectionControlRegionBTagAlpha::process(Event & event) {
    ...
    double d_alpha = alpha->Eval(ST);

    event.weight *= d_alpha;



    h_finalselection_alpha->fill(event);
    h_jets_finalselection_alpha->fill(event);
    h_ele_finalselection_alpha->fill(event);
    h_mu_finalselection_alpha->fill(event);
    h_event_finalselection_alpha->fill(event);
    h_lumi_finalselection_alpha->fill(event);
  ...
  UHH2_REGISTER_ANALYSIS_MODULE(LQTopMuRun2FullSelectionControlRegionBTagAlpha)
