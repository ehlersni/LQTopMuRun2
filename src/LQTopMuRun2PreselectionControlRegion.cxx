...
namespace uhh2examples {
  ...
  class LQTopMuRun2PreselectionControlRegion: public AnalysisModule {
  public:

    explicit LQTopMuRun2PreselectionControlRegion(Context & ctx);
  ...
  private:
    ...
    unique_ptr<Hists> h_nelectrons, h_jets_nelectrons, h_ele_nelectrons, h_mu_nelectrons, h_event_nelectrons, h_lumi_nelectrons;
    ...
    unique_ptr<Selection> muon_selection, jet_selection, st_selection, electron_selection;
  ...
  LQTopMuRun2PreselectionControlRegion::LQTopMuRun2PreselectionControlRegion(Context & ctx){
    cout << "Hello World from LQTopMuRun2PreselectionControlRegion!" << endl;
    ...
    // ElectronID = AndId<Electron>(ElectronID_Spring16_tight, PtEtaCut(30., 2.4));

    Year year = extract_year(ctx);
    if (year == Year::is2016v3) {
      ElectronID = AndId<Electron>(ElectronID_Summer16_tight, PtEtaCut(30.0, 2.4));
    }
    else if (year == Year::is2017v1) {
      ElectronID = AndId<Electron>(ElectronID_Fall17_tight, PtEtaCut(30.0, 2.4));
    }
    ...
    h_nelectrons.reset(new LQTopMuRun2Hists(ctx, "NElectrons"));
    h_jets_nelectrons.reset(new JetHists(ctx, "Jets_NElectrons"));
    h_ele_nelectrons.reset(new ElectronHists(ctx, "Ele_NElectrons"));
    h_mu_nelectrons.reset(new MuonHists(ctx, "Mu_NElectrons"));
    h_event_nelectrons.reset(new EventHists(ctx, "Event_NElectrons"));
    h_lumi_nelectrons.reset(new LuminosityHists(ctx, "Lumi_NElectrons"));
  ...
  */
  ...
  bool LQTopMuRun2PreselectionControlRegion::process(Event & event) {
    ...
    h_nelectrons->fill(event);
    h_jets_nelectrons->fill(event);
    h_ele_nelectrons->fill(event);
    h_mu_nelectrons->fill(event);
    h_event_nelectrons->fill(event);
    h_lumi_nelectrons->fill(event);
  ...
  UHH2_REGISTER_ANALYSIS_MODULE(LQTopMuRun2PreselectionControlRegion)
